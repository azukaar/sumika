...
type TimezoneSearchResponse struct {
	Results []TimezoneInfo `json:"results"`
	Total   int            `json:"total"`
}

// IANA timezone list - major timezones (can be extended)
var ianaTimezones = []string{
	...
	"Africa/Cairo", "Africa/Casablanca", "Africa/Conakry", "Africa/Dakar", "Africa/Dar_es_Salaam", "Africa/Djibouti",
...
func NewTimezoneService() *TimezoneService {
	...
	service.initializeTimezones()
	return service
}

func (s *TimezoneService) initializeTimezones() {
	s.timezones = make([]TimezoneInfo, len(ianaTimezones))
	...
	for i, tz := range ianaTimezones {
		...
		displayName := fmt.Sprintf("%s (%s)", city, region)
		if tz == "UTC" {
			displayName = "UTC (Coordinated Universal Time)"
		}
		
		s.timezones[i] = TimezoneInfo{
			ID:          tz,
	...
	sort.Slice(s.timezones, func(i, j int) bool {
		return s.timezones[i].DisplayName < s.timezones[j].DisplayName
	})
}

// SearchTimezones searches for timezones by name or region
func (s *TimezoneService) SearchTimezones(query string, limit int) (*TimezoneSearchResponse, error) {
	context := utils.NewLogContext("timezone").WithOperation("search_timezones")
	
	query = strings.ToLower(strings.TrimSpace(query))
	if query == "" {
		// Return all timezones if no query
		results := s.timezones
		...
		return &TimezoneSearchResponse{
			Results: results,
			Total:   len(s.timezones),
		}, nil
	}
	
	var matches []TimezoneInfo
	for _, tz := range s.timezones {
		// Search in ID, display name, region, and city
		if strings.Contains(strings.ToLower(tz.ID), query) ||
			strings.Contains(strings.ToLower(tz.DisplayName), query) ||
			strings.Contains(strings.ToLower(tz.Region), query) ||
			strings.Contains(strings.ToLower(tz.City), query) {
	...
	utils.InfoWithContext(fmt.Sprintf("Found %d timezone matches for query '%s'", len(matches), query), context)
...
// GetAllTimezones returns all available timezones
func (s *TimezoneService) GetAllTimezones() *TimezoneSearchResponse {
	return &TimezoneSearchResponse{
		Results: s.timezones,
		Total:   len(s.timezones),
	}
...
// IsValidTimezone checks if a timezone ID is valid
func (s *TimezoneService) IsValidTimezone(timezoneID string) bool {
	for _, tz := range s.timezones {
		if tz.ID == timezoneID {
			return true
		}
